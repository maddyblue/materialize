...
use crate::coord::{
    ...
    PeekStageTimestampReadHold, PlanValidity, RealTimeRecencyContext, StageResult, Staged,
    TargetCluster,
};
...
impl Staged for PeekStage {
    ...
    async fn stage(
        ...
        match self {
            ...
            PeekStage::ExplainPlan(stage) => coord.peek_explain_plan(ctx.session(), stage),
            PeekStage::ExplainPushdown(stage) => {
                coord.peek_explain_pushdown(ctx.session(), stage).await
            }
            PeekStage::CopyTo(stage) => coord.peek_copy_to_dataflow(stage).await,
        }
    }
    ...
    async fn peek_explain_pushdown(
        ...
        Ok(StageResult::HandleRetire(mz_ore::task::spawn(
            ...
            async move {
                ...
                match tokio::time::timeout(explain_timeout, futures.try_collect::<Vec<_>>()).await {
