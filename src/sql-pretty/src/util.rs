...
impl Prettier {
    pub(crate) fn comma_separate<'a, F, T: 'a, I>(&'a self, f: F, v: I) -> RcDoc<'a, ()>
    where
        F: Fn(&'a Self, &'a T) -> RcDoc<'a, ()>,
        I: IntoIterator<Item = &'a T>,
    {
        let docs = v.into_iter().map(|d| f(self, d));
    ...
    where
        ...
        I: IntoIterator<Item = &'a T>,
    {
        nest(title, self.comma_separate(f, v))
    }
}
