            ...
            let (output, desc) = match parts {
                Some(("static" | "dynamic", entity)) => {
                    ...
                    let Some((output, desc)) = LDBC_DESCS.get(entity.as_str()) else {
                        tracing::error!("could not find ldbc relation: {entity}");
                        continue;
                    };
                    (output, desc)
                }
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            match self.next_record() {
                Some((output, row)) => {
                    return Some((output, GeneratorMessageType::InProgress, row, 1))
                }
                None => {
                    // If we're out of records for this file, open the next and try again.
                    self.rdr = None;
                    self.next_file();
                    // No more files.
                    if self.rdr.is_none() {
