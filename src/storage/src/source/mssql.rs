...
use mz_mssql_util::desc::PostgresTableDesc;
use mz_mssql_util::PostgresError;
...
use tokio_mssql::error::SqlState;
use tokio_mssql::types::PgLsn;
use tokio_mssql::{Client, SimpleQueryMessage, SimpleQueryRow};
...
pub enum TransientError {
    ...
    SQLClient(#[from] tokio_mssql::Error),
    #[error(transparent)]
...
async fn fetch_slot_resume_lsn(client: &Client, slot: &str) -> Result<MzOffset, TransientError> {
    loop {
        ...
        match row.get("confirmed_flush_lsn") {
            // For mssql, `confirmed_flush_lsn` means that the slot is able to produce
