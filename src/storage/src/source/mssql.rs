...
use mssql_protocol::message::backend::{LogicalReplicationMessage, ReplicationMessage, TupleData};
...
use tokio_mssql::error::DbError;
use tokio_mssql::replication::LogicalReplicationStream;
use tokio_mssql::types::PgLsn;
use tokio_mssql::Client;
use tokio_mssql::SimpleQueryMessage;
use tracing::{info, warn};

use mz_expr::MirScalarExpr;
use mz_mssql_util::desc::MssqlTableDesc;
...
use mz_storage_client::types::sources::{MssqlSourceConnection, MzOffset, SourceTimestamp};
...
/// Mssql epoch is 2000-01-01T00:00:00Z
static MS_EPOCH: Lazy<SystemTime> = Lazy::new(|| UNIX_EPOCH + Duration::from_secs(946_684_800));
...
impl MsErrorExt for tokio::time::error::Elapsed {
    fn is_definite(&self) -> bool {
        false
    }
}

impl MsErrorExt for DbError {
    fn is_definite(&self) -> bool {
        let class = match self.code().code().get(0..2) {
            None => return false,
            Some(class) => class,
        };
        // See https://www.mssqlql.org/docs/current/errcodes-appendix.html for the class
        // definitions.
        match class {
...
impl MsErrorExt for tokio_mssql::Error {
    fn is_definite(&self) -> bool {
        match self.source() {
...
impl MsErrorExt for std::io::Error {
    fn is_definite(&self) -> bool {
        match self.source() {
            Some(err) => match err.downcast_ref::<tokio_mssql::Error>() {
...
impl<E: MsErrorExt + Into<anyhow::Error>> From<E> for ReplicationError {
    fn from(err: E) -> Self {
        if err.is_definite() {
...
/// Information required to sync data from Mssql
pub struct MssqlSourceReader {
...
/// An OffsetCommitter for mssql, that sends
...
struct SourceTable {
    ...
    desc: MssqlTableDesc,
    /// The scalar expressions required to cast the text encoded columns received from mssql
    /// into the target relational types
    casts: Vec<MirScalarExpr>,
}

/// An internal struct held by the spawned tokio task
struct MssqlTaskInfo {
    source_id: GlobalId,
    connection_config: mz_mssql_util::Config,
    publication: String,
    slot: String,
...
impl SourceRender for MssqlSourceConnection {
    type Key = ();
    type Value = Row;
    type Time = MzOffset;
    ...
    fn render<G: Scope<Timestamp = MzOffset>>(
        ...
        let button = builder.build(move |mut capabilities| async move {
            ...
            task::spawn(|| format!("mssql_source:{}", config.id), {
                mssql_replication_loop(task_info)
            });
            ...
            let mut reader = MssqlSourceReader {
...
/// Defers to `mssql_replication_loop_inner` and sends errors through the channel if they occur
#[allow(clippy::or_fun_call)]
async fn mssql_replication_loop(mut task_info: MssqlTaskInfo) {
    loop {
        ...
        match mssql_replication_loop_inner(&mut task_info).await {
            Ok(()) => {}
...
async fn mssql_replication_loop_inner_snapshot(
    task_info: &mut MssqlTaskInfo,
    ...
    let publication_tables =
        mz_mssql_util::publication_info(&task_info.connection_config, &task_info.publication, None)
            .await
            .err_indefinite()?;
    ...
    // abundance of caution, as rust-mssql has had curious bugs around this.
    drop(stream);
    drop(client);
    ...
    {
        tracing::info!(
            "mssql snapshot was at {snapshot_lsn:?} but we need it at {slot_lsn:?}. Rewinding"
        );
...
/// Core logic
async fn mssql_replication_loop_inner(
    task_info: &mut MssqlTaskInfo,
) -> Result<(), ReplicationError> {
    if task_info.replication_lsn == PgLsn::from(0) {
        match mssql_replication_loop_inner_snapshot(task_info).await {
...
/// Determines if a set of [`SourceTable`]s and a set of [`MssqlTableDesc`] are compatible with
...
fn determine_table_compatibility<'a, I>(
    source_tables: I,
    tables: Vec<MssqlTableDesc>,
) -> Vec<(u32, usize, anyhow::Error)>
where
    I: Iterator<Item = (&'a u32, &'a SourceTable)>,
{
    let pub_tables: BTreeMap<u32, MssqlTableDesc> =
...
async fn produce_replication<'a>(
    client_config: mz_mssql_util::Config,
    slot: &'a str,
    publication: &'a str,
    as_of: PgLsn,
    ...
    async_stream::try_stream!({
        ...
        loop {
            ...
            loop {
                ...
                // startup.
                //
                // See: https://www.mssqlql.org/message-id/CAMsr+YE2dSfHVr7iEv1GSPZihitWX-PMkD9QALEGcTYa+sdsgg@mail.gmail.com
                    ...
                    Ok(Some(XLogData(xlog_data))) => match xlog_data.data() {
                        ...
                        Relation(relation) => {
                            ...
                            if let Some(info) = source_tables.get(&rel_id) {
                                ...
                                let current_publication_info = mz_mssql_util::publication_info(
                                    &client_config,
                                    publication,
                                    Some(rel_id),
                                )
            ...
            // `mssql_replication_loop_inner`, we drop clients aggressively out of caution.
            drop(stream);
...
async fn peek_wal_lsns(
    config: mz_mssql_util::Config,
    slot: &str,
    publication: &str,
    up_to: Option<PgLsn>,
) -> Result<impl Iterator<Item = PgLsn>, mz_mssql_util::MssqlError> {
