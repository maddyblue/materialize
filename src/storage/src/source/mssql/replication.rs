...
//! Renders the logical replication side of the [`MssqlSourceConnection`] ingestion dataflow.
//!
//! ```text
//!              o
//!              │rewind
...
use mz_storage_client::types::sources::{MssqlSourceConnection, MzOffset};
use mz_timely_util::builder_async::{Event as AsyncEvent, OperatorBuilder as AsyncOperatorBuilder};
use mz_timely_util::operator::StreamExt as TimelyStreamExt;

use crate::source::mssql::verify_schema;
use crate::source::mssql::{metrics::PgSourceMetrics, DefiniteError, TransientError};
...
pub(crate) fn render<G: Scope<Timestamp = MzOffset>>(
    ...
    connection: MssqlSourceConnection,
    context: ConnectionContext,
    resume_upper: Antichain<MzOffset>,
    table_info: BTreeMap<u32, (usize, MssqlTableDesc, Vec<MirScalarExpr>)>,
...
fn raw_stream<'a>(
    ...
    connection_config: &'a mz_mssql_util::Config,
    ...
    async_stream::try_stream!({
        ...
        // Mssql will return all transactions that commit *strictly* after the provided LSN but
        ...
        // [1] https://www.mssqlql.org/docs/15/protocol-replication.html#PROTOCOL-REPLICATION-START-REPLICATION-SLOT-LOGICAL
...
fn extract_transaction<'a>(
    ...
    table_info: &'a BTreeMap<u32, (usize, MssqlTableDesc, Vec<MirScalarExpr>)>,
    connection_config: &'a mz_mssql_util::Config,
            ...
            match event {
                ...
                Relation(body) => {
                    ...
                    if let Some((_, expected_desc, _)) = table_info.get(&rel_id) {
                        ...
                        let upstream_info = mz_mssql_util::publication_info(
                            connection_config,
                            publication,
                            Some(rel_id),
                        )
